...
- [x] 增加指定目录 SHA-256 重复图片试扫，且与全图库扫描共用 worker、IPC、单写入者和锁恢复管线
  - 状态：2026-08-01 已完成 TEMP 目录、重复组、锁等待、停止和互斥回归；正式目录验收和发布记录见 `HANDOFF.md`。

- [x] 增加重复图片 SHA-256 扫描的实时、可恢复任务进度；本次任务统计和数据库累计统计分离
  - 状态：2026-08-01隔离验证和正式只读浏览器验收通过，已部署v109；不改变SHA-256、pHash、重复规则、正式媒体或数据库。

- [x] 修复关闭懒加载后详情图集不能自动完整加载
  - 状态：v108以当前详情会话和5并发预览队列完成本图集图片，不预取相邻图集或视频；路由切换、重新渲染和重新开启懒加载会取消未开始任务及当前分页fetch。TEMP 100/300图片浏览器验证通过。

- [x] 修复显示设置重复渲染后内容消失
  - 状态：v107在设置页重渲染前复用既有工具栏还原逻辑，避免替换`#view`时删除显示设置控件；只修改前端挂载时序并补充回归断言。

- [x] 整合媒体库优化设置入口并解除目录刷新页面阻塞
  - 状态：v106已将四项一级入口收敛为`媒体库优化`，旧地址兼容；首屏只读汇总，目录刷新仅用户触发，既有后台任务互斥保持生效。SQLite统一5秒忙等待，pHash保持10条短事务并对忙锁有限重试；TEMP SQLite与隔离HTTP验证通过，未接触正式媒体或数据库。

- [x] 修复Windows媒体句柄导致的图集定时回收失败
  - 状态：已发布`v104-20260722-2129`。所有媒体文件响应统一清理并登记Node流；`EPERM/EBUSY`按5分钟最多12次持久重试；失败状态显示原因、占用文件、普通重试和仅释放本服务流的强制重试。TEMP隔离测试、正式数据库迁移及旧失败任务只读页面验收通过。

- [x] 图集回收按钮改为无确认弹窗的直接标记
  - 状态：末级纯媒体图集点击“回收”后直接标记；服务端资格/权限校验和至少60分钟撤销期保持不变，到期后仅在每日04:00停站维护窗口执行；白天可取消，不提供强制释放或手工重试。
