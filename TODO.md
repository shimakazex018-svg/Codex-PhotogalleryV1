...
- [x] 修复正式Node重启后媒体清理历史报告不可见；v89只恢复最新有效报告用于查看，并保持恢复报告删除禁用（正式v89重启待执行）。
- [x] 将首页收藏/最近观看迁入设置子页，并把访问日志迁入SQLite，完成50条服务端分页、365天低频保留清理和旧NDJSON幂等导入（前端`v88`；正式Node后端重启与实体iPad/iPhone复验尚未执行）。
