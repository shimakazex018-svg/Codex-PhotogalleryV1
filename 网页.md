...
> 正式收敛说明（2026-07-22）：此前v102功能线与main v96新增功能线已经通过独立集成Worktree进行可追溯合并。统一版本同时保留FTS5、8种排序、上传图片SHA/pHash查找、视频兼容、时间戳版本记录、统一LAN/ZeroTier管理权限、每日04:00检查和图集延迟回收；main在发布后作为唯一正式源码。全过程未移动或删除正式媒体。

> 新项目基线说明（2026-07-11）：当前唯一维护目录为 `<project-root>`。本项目从 `<legacy-project-root>` 的实际运行工作树选择性迁移，旧目录保持只读。当前源码和正式前端版本为`v98`；SQLite访问日志分页、媒体清理历史恢复、可恢复回收流程、FTS5搜索、只读视频兼容性扫描与报告驱动兼容流均已启用。生产数据库、媒体、日志和生成缓存未迁入 Git，运行时通过 `PHOTOS_DIR`、`DATA_DIR` 等环境变量挂载。迁移来源和文件映射以 `docs/MIGRATION_SOURCE.md`、`docs/MIGRATION_MANIFEST.md` 为准。下文保留迁移前审计数据用于追溯，其中路径、统计数量、进程状态和 `v69` 等时间点信息不代表新项目当前状态。

> 当前增量（2026-07-15）：`v91`已把媒体清理的v86永久删除改为项目回收站。设置页`#/__settings/media-cleanup`通过`/api/media-cleanup/recycle`与`/restore`执行localhost显式确认；旧`/delete`返回410。源和目标只由服务端`PHOTOS_DIR`、`TRASH_DIR`与批准报告解析，完整说明见`docs/MEDIA_LIBRARY_CLEANUP_SETTINGS.md`。
...
> 当前正式版本（2026-07-22）：`v103-20260722-1209`统一收敛此前v102与main v96功能线并在48102发布。自v102起完整版本号包含正式发布完成时的Asia/Shanghai日期和时分，页面版本与全部静态资源缓存参数保持一致。
> 当前增量（2026-07-18）：设置页新增`#/__settings/release-notes`，按需读取`release-notes.json`并默认显示最近20个版本；页脚版本号可直接进入。历史v99、v100、v101只补录已确认日期，不伪造时分；详细规范见`docs/RELEASE_VERSIONING.md`。
> 当前增量（2026-07-18）：v101保留v100签名优先校验和v99排序；SHA-256定义“完全相同”，pHash距离0-6显示“高度相似”、7-10显示“可能相似”。缩略、重压缩、改尺寸和格式转换为目标能力；大幅裁剪、镜像、旋转、人脸或语义搜索不在保证范围。
...
| 搜索 API | `GET /api/search?q=杨晨晨&limit=5` | 200；v96附带mode/index/degraded状态 |
...
1. 在顶部搜索框输入关键词。
2. 至少输入2个字符；前端等待250ms后调用 `/api/search?q=<关键词>&limit=60`，新输入会取消旧请求。
3. 搜索结果分为目录和媒体，总数最多60；精确/前缀图集优先。
4. 相同关键词30秒内复用内存结果；清空搜索框后恢复当前页面列表。
5. 正式v96在索引ready时使用mapped trigram FTS；`auto`不会回退到完整媒体LIKE扫描。
...
5. `利世/.../看球`的旧`mpeg4/mp4v`视频在点击后使用单路H.264/AAC兼容流；暂停或离页会停止转码，其他图集不受影响。
...
| 图集页 | 旧编码兼容视频 | 指定`看球`图集 | 有旧`mpeg4/mp4v`视频 | 点击播放 | 单路实时输出H.264/AAC，暂停/切换/离页终止 | `renderVideos()`、`sendCompatibleVideo()` | `/api/video-compatible`、`/api/video-compatible/stop` | 无 | 临时CPU/带宽，不落盘 | 转码失败或不支持拖动 |
...
| 搜索 | 搜索框 | `app.js`、`gallery-db.js` | `setSearchQuery()`、`requestSqliteSearch()`、`search()` | 250ms防抖input | `/api/search` | 搜索 API/执行计划/浏览器隔离验证 | 图集精确/前缀走索引；媒体包含仍可能全表扫描 |
...
| GET | `/api/search` | 搜索 | `q,limit`（默认50/最大60；最少2字符） | `collections,media,hasMore,limit` | 无 | `requestSqliteSearch()` | `galleryDb.search()` | 实际计划与真实数据副本已验证 |
...
- 设置页管理写操作通过`/api/admin/capabilities`按local、trusted-lan、trusted-zerotier启用；远程“打开所在位置”仍不可用。
- 末级纯媒体图集标题右侧显示“回收”：点击后不弹确认框，直接创建待回收标记；仍至少保留1小时撤销期，并显示标记、最早和计划整点，可再次点击取消。
- 每个整点批量移动到`E:\回收站`相同相对目录，目标冲突不覆盖；Windows返回`EPERM/EBUSY`时每5分钟自动重试、最多12次，失败页面继续显示原因、Node占用文件、普通重试和“强制释放并回收”。
- “强制释放并回收”只销毁目标图集下由当前图库Node登记的HTTP媒体流，不扫描或关闭其他进程句柄，也不终止Node；04:00批次结束后仍执行每日索引刷新。
- 一级、二级、三级卡片封面不再显示红底白字名称，下方标题、计数、视频功能标识不变。
