...
> 新项目基线说明（2026-07-11）：当前唯一维护目录为 `<project-root>`。本项目从 `<legacy-project-root>` 的实际运行工作树选择性迁移，旧目录保持只读。当前源码和正式前端版本为`v91`；SQLite访问日志分页、媒体清理历史恢复与可恢复回收流程均已启用。生产数据库、媒体、日志和生成缓存未迁入 Git，运行时通过 `PHOTOS_DIR`、`DATA_DIR` 等环境变量挂载。迁移来源和文件映射以 `docs/MIGRATION_SOURCE.md`、`docs/MIGRATION_MANIFEST.md` 为准。下文保留迁移前审计数据用于追溯，其中路径、统计数量、进程状态和 `v69` 等时间点信息不代表新项目当前状态。
...
> 当前候选（2026-07-16）：源码v96已接入稳定mapping+trigram FTS、严格`auto/fts5/legacy-like`模式、索引状态API和显式迁移/校验工具；2字符只搜媒体标题精确/前缀，3字符以上ready时使用FTS。完整数据库副本和隔离API已通过，但正式库/48102未迁移或部署；真实Chrome和完整隔离媒体树扫描峰值未完成，候选不可正式部署。详见`docs/SEARCH_FTS5_INTEGRATION_V96.md`。
...
| 搜索 API | `GET /api/search?q=杨晨晨&limit=5` | 200；候选v96附带mode/index/degraded状态 |
...
5. FTS5 Prototype V96目前仅为离线副本方案；正式页面和`/api/search`仍使用v95流程，尚未发布v96。
