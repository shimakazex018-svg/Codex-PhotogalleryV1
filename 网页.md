...
> 当前正式版本（2026-07-18）：前端v99已在48102部署；设置页`#/__settings/video-compatibility`继续管理只读分层扫描，正式2096条视频已分类为1432直接播放、267设备依赖、395兼容流、2异常。兼容流只由报告中的媒体ID触发且单路无落盘。搜索仍采用v96稳定mapping+trigram FTS，正式索引ready且`auto`实际使用FTS5；2字符只搜媒体标题精确/前缀，3字符以上使用FTS。
> 当前增量（2026-07-18）：v99把图册排序统一为名称、图片数、视频数、内容更新时间的正序/倒序，根目录在后端排序后分页；搜索默认仍按FTS相关性。顶部“上传图片查找”以原始字节SHA-256查询既有`media_hashes`索引，单文件流式处理且不落盘，只返回图库相对路径；它不提供相似图、裁剪、压缩或格式转换后的匹配。
