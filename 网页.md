...
> 当前正式版本（2026-07-16）：前端v98已在48102部署；设置页`#/__settings/video-compatibility`管理只读分层扫描，正式2096条视频已分类为1432直接播放、267设备依赖、395兼容流、2异常。兼容流只由报告中的媒体ID触发且单路无落盘。搜索仍采用v96稳定mapping+trigram FTS，正式索引ready且`auto`实际使用FTS5；2字符只搜媒体标题精确/前缀，3字符以上使用FTS。
