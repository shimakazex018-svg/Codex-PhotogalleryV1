...
> 当前正式版本（2026-07-18）：前端v100已在48102部署；设置页`#/__settings/video-compatibility`继续管理只读分层扫描，正式2096条视频已分类为1432直接播放、267设备依赖、395兼容流、2异常。兼容流只由报告中的媒体ID触发且单路无落盘。搜索仍采用v96稳定mapping+trigram FTS，正式索引ready且`auto`实际使用FTS5；2字符只搜媒体标题精确/前缀，3字符以上使用FTS。
> 当前增量（2026-07-18）：v100修复“上传图片查找”的PNG/MIME/文件签名识别；文件签名为主判据，浏览器MIME仅作辅助，扩展名冲突明确返回声明格式和真实格式。v99的8种排序和原始字节SHA-256精确查找保持不变，不提供相似图、裁剪、压缩或格式转换后的匹配。
