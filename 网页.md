...
> 当前增量（2026-07-26）：四项维护工具统一位于`#/__settings/media-optimization`；页面进入只读轻量状态，目录刷新、查重、pHash、清理与视频检查均由用户显式启动。旧工具hash保持兼容并定位到对应区块；`/api/media-optimization/status`不会触发扫描。
