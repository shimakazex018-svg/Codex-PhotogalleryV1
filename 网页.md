...
> 当前候选（2026-07-16）：源码v96保留稳定mapping+trigram FTS、严格`auto/fts5/legacy-like`模式、基本索引状态和最小迁移/备份/quick-full校验工具；2字符只搜媒体标题精确/前缀，3字符以上ready时使用FTS。正式库/48102未迁移或部署，生产级扩展与Chrome自动验收不再列为后续门槛。详见`docs/SEARCH_FTS5_INTEGRATION_V96.md`。
...
5. 源码候选v96在索引ready时使用mapped trigram FTS；正式运行站仍为v91，尚未发布v96。
