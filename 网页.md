...
> 新项目基线说明（2026-07-11）：当前唯一维护目录为 `D:\A8 Codex\Codex-PhotogalleryV1`。本项目从旧目录 `D:\A8 Codex\Codex-Photogallery\photo-gallery-site` 的实际运行工作树选择性迁移，旧目录保持只读。当前前端版本为 `v70`。生产数据库、媒体、日志和生成缓存未迁入 Git，运行时通过 `PHOTOS_DIR`、`DATA_DIR` 等环境变量挂载。迁移来源和文件映射以 `docs/MIGRATION_SOURCE.md`、`docs/MIGRATION_MANIFEST.md` 为准。下文保留迁移前审计数据用于追溯，其中路径、统计数量、进程状态和 `v69` 等时间点信息不代表新项目当前状态。
...
|---|---|
| 文档生成日期 | 2026-07-10 |
| 工作区根目录 | `D:\A8 Codex\Codex-Photogallery` |
| 实际网站 Git 根目录 | `D:\A8 Codex\Codex-Photogallery\photo-gallery-site` |
| 文档输出路径 | `D:\A8 Codex\Codex-Photogallery\网页.md` |
...
2. 首页上方显示搜索框和设置按钮。
3. 页面主体显示同步状态、面包屑、亮点轮播、收藏、最近观看、模特/一级目录网格。
4. 点击模特卡片进入对应目录。
5. 点击轮播图会跳转到该图片所属图集。
...
- 显示最近访问记录。
- 记录时间、IP、类型、模特、图集。
- 点击刷新日志调用 `/api/access-log?limit=100`。

## 6. 页面与路由总览

| 页面名称 | 路由 | 入口文件 | Layout | 访问条件 | 用途 | 数据来源 | 当前状态 |
|---|---|---|---|---|---|---|---|
| 首页 | `#/` | `index.html` + `app.js` | `header.topbar` + `main.page` | 无登录要求 | 浏览一级目录、轮播、收藏、最近观看 | `/api/collections/root`、`/api/highlights`、`/api/recent`、`/api/favorites` | 已验证 |
...
- 页面用途：展示一级目录、轮播、收藏、最近观看。
- 访问条件：无需登录。
- 数据来源：`/api/collections/root`、`/api/highlights`、`/api/recent`、`/api/favorites`。
- 加载流程：`loadGallery()` 调用 SQLite root 和 highlights；`renderModels()` 渲染。
- 主要区域：顶部搜索与设置、状态栏、面包屑、轮播、收藏、最近观看、目录网格。
- 可跳转页面：任意目录页、设置页。
...
flowchart LR
  ...
  API --> Photos[E:\A_秀人 或 PHOTOS_DIR]
  API --> Data[data 目录]
...
| 访问日志 | `#/__settings/access-log` | `app.js`、`server.js` | `appendAccessLog()`、`readAccessLogs()` | 页面访问 | 日志文件 | API 200 | 日志保留 14 天 |
...
| `server.js` 中 `ensureHighlightCarousel(gallery)` | 疑似旧轮播实现 | 当前未发现调用 | 当前 `/api/highlights` 走 `ensureHighlightCarouselFromDb()` | 基于内存 gallery 的轮播 | 误调用会重新依赖大对象 | 暂时保留，后续清理前跑 rg |
...
| `README.md` | 文档过期 | 否 | 仍描述 `localhost:5177` 和旧目录扫描 | 初版说明 | 误导新维护者 | 以本文档和 `README-SERVER-WINDOWS.md` 为准 |
...
| 灯箱状态 | `lightboxImages`、`lightboxIndex`、缩放和拖拽坐标 |
...
| GET/POST | `/api/access-log` | 访问日志 | limit/item | 日志 | 无 | 设置页、页面访问记录 | `readAccessLogs()`、`appendAccessLog()` | 已验证 |
...
| 类型 | 路径或配置 | 说明 |
|---|---|---|
| 图片/视频源目录 | `PHOTOS_DIR`，默认 `photo-gallery-site/photos`，当前运行疑似 `E:\A_秀人` | 原始媒体文件 |
...
| 访问日志 | `data/logs/access-YYYY-MM-DD.log` | 保留约 14 天 |
...
| `PHOTOS_DIR` | 媒体源目录 | `server.js` | 否 | `./photos` | 外部大盘目录 | `E:\A_秀人` |
| `DATA_DIR` | 数据目录 | `server.js` | 否 | `./data` | 建议持久化目录 | `D:\sites\photo-gallery-site\data` |
...
| `TRASH_DIR` | 删除重复图片目标 | `server.js` | 否 | `PHOTOS_DIR` 同级 `回收站` | 建议与源文件同盘 | `E:\回收站` |
...
| 访问日志 | `data/logs/access-YYYY-MM-DD.log` | 时间、IP、host、userAgent、类型、标题、路径 |
...
1. 先读本文档。
2. 确认实际 Git 根目录是 `D:\A8 Codex\Codex-Photogallery\photo-gallery-site`。
