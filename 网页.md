...
- 白天只写回收标记和取消；满足至少60分钟后，在下一次04:00维护窗口停止图库、确认48102退出，再移动到`E:\回收站`相同相对目录，目标冲突不覆盖。
- 维护失败会保留原因和次数，下一次维护窗口再试；不使用外部句柄工具、不终止无关Node、不提供“强制释放并回收”。网站恢复HTTP 200后才启动现有索引扫描。
