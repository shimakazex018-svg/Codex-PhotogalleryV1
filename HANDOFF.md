...
已发布`v106-20260726-0909`：四项媒体库工具收敛为`媒体库优化`，设置页框架即时渲染、目录刷新改为显式后台操作，旧hash继续兼容。统一状态API只读汇总SQLite和任务快照；常规连接为WAL + 5000ms busy timeout，pHash仍使用10条短事务并有限退避。TEMP SQLite、隔离HTTP和三视口浏览器验证通过；正式48102已精确重启、三入口HTTP 200、未触碰正式媒体。
