...
修复`杏子yada/亮点`灯箱空白：确认7个`.jpg`源文件实际为HEIC内容，仅为该图集的灯箱使用既有WebP兼容预览；媒体原文件、数据库、服务端路由和其他图集行为均未修改，前端版本为`v80`。

## Current State

- 业务基线：`v1.3-release` 已发布到 GitHub。
- 当前分支：`main`。
- 前端版本`v80`；列表使用独立按需WebP预览，`杏子yada/亮点`灯箱也使用同一兼容预览以绕过伪装成JPEG的HEIC源文件，其他灯箱仍加载原图。首页/媒体limit为40，图片DOM首批24，视频始终preload=none。页面支持安全区、空闲弱化、可中断S曲线回顶动画，以及有界的会话级滚动恢复。
