...
实现 hash SPA 的滚动位置恢复：浏览器后退/前进、父级面包屑、搜索结果、收藏、最近观看和刷新会回到原可见元素；普通进入新页面仍从顶部开始。深层媒体只按既有分页补齐到保存位置，前端版本为`v79`。

## Current State

- 业务基线：`v1.3-release` 已发布到 GitHub。
- 当前分支：`main`。
- 前端版本`v79`；列表使用独立按需WebP预览，首页/媒体limit为40，图片DOM首批24，视频始终preload=none。页面支持安全区、空闲弱化、可中断S曲线回顶动画，以及有界的会话级滚动恢复。
