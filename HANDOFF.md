...
- 源码与正式运行站均为`v101`，Node PID 19540，监听`0.0.0.0:48102`；loopback和`192.168.31.153`均HTTP 200且只有一个48102监听。
...
5. FTS阶段A原始副本和JSON位于Git忽略的`tmp/fts5-prototype`；它们可重建且不应提交。正式Node PID为19540，端口仍为48102；pHash 10,000张受限批次可从设置页暂停或停止。
