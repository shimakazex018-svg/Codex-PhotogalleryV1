...
### Impact
普通预加载仍最大并发2、缓存最多5项并保留Save-Data/2G/3G策略；关闭或跨图集会停止旧任务，失败只允许有限重试。调试日志默认关闭，可通过`galleryLightboxDebug=1`或`?lightboxDebug=1`临时启用。服务端原图缓存、API、数据库、视频/HLS和视觉布局不变。

### Status
有效，前端`v85`实施；真实Chrome已完成请求顺序、复用、连续切换和关闭任务专项验收，Disable cache/HAR、亚秒级快速连点、节流和长期内存趋势仍待补。
