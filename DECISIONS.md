...
保留现有预览URL、分批DOM和`IntersectionObserver`；关闭懒加载时不预取相邻图集、不请求原图或完整视频，而是为当前详情页建立一次性内存会话，以5并发激活同一预览资源并逐帧追加该图集剩余图片节点。路由离开、重新渲染或重新开启懒加载时取消未开始队列、观察器、动画帧及关联分页fetch。

### Reason

仅切换配置或`loading`属性不能激活未进入观察范围的节点，且原实现只插入首批图片。无限并发会同时占用网络、解码与主线程；直接`<img>`请求不能在Windows/浏览器侧可靠强杀，必须用会话隔离阻止旧结果污染新页面。

### Impact

关闭时当前图集最终会自动请求全部预览图，但任何时刻最多5张由本队列激活；视频保持`preload="none"`。大图集的DOM仍按批次产生，快速切换不会累积旧队列或旧分页响应。

### Status

有效；自v108-20260727-2101起使用。

## DEC-030：媒体库优化统一入口与短事务锁策略

### Decision

保留既有四个任务实现和API，不新增第二套持久任务系统；设置页统一为按需加载的`媒体库优化`工作区，旧hash保持兼容。SQLite读取统一使用只读连接，所有常规连接设置`busy_timeout=5000`；pHash继续在文件计算之外以10条短写事务提交，并只对`SQLITE_BUSY`有限退避5次。

### Reason

避免路由渲染等待大结果或扫描，降低读写锁争用，同时不改变媒体、现有任务语义或正式部署结构。

### Impact

媒体库优化首屏只读取一个汇总状态接口；任务互斥仍由既有服务端门禁负责。pHash已提交批次保持有效，服务重启后的遗留运行状态向用户显示为`interrupted`，可由用户明确继续。

### Status

有效；自v106-20260726-0909起使用。
