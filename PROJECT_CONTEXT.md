...
- 当前源码前端版本为`v109-20260801-1230`（`app.js`的`APP_VERSION`及全部静态资源缓存参数一致）；正式Runtime仍以实际部署版本为准。
...
- 正式Runtime已运行`v107-20260726-0922`，Node PID 20412、Host PID 30368；每日03:59:50维护任务已安装，正式dry-run通过且未停止网站、未改写队列或移动媒体。当前队列属于可变化的运行状态，以API为准。
...
- 精确 SHA-256 查重任务在`DATA_DIR/duplicate-scan-status.json`保存有界实时状态；本次任务进度与数据库累计哈希统计分离。worker只读文件并经IPC回传，主服务以单一、有界队列批量写入；SQLite暂态锁显示为等待/重试，不计为坏图片。服务重启时遗留活动任务显示为`interrupted`，停止请求会排空已计算队列后安全完成。查重入口支持全图库`scope=all`和媒体根目录内的`scope=directories`；后者只改变候选提供器，完成后把限定候选的重复组写入`DATA_DIR/reports/duplicate-scope-scan-<jobId>.json`。
...
- Windows Task Scheduler于03:59:50运行独立维护编排器：锁防重入、精确停止48102、离线回收、启动并确认loopback HTTP 200、最后启动既有索引扫描。维护期间48102可短暂不可访问。
- `collection_recycle_queue`使用`waiting`、`ready-for-maintenance`、`recycling`、`recycled`和`recycle_failed`；旧失败历史保留且不自动转为待回收。
- 可选手工 HLS 生成与静态访问。
