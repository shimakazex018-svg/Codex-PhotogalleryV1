...
- 精确 SHA-256 查重任务在`DATA_DIR/duplicate-scan-status.json`保存有界实时状态；本次任务进度与数据库累计哈希统计分离。worker只读文件并经IPC回传，主服务以单一、有界队列批量写入；SQLite暂态锁显示为等待/重试，不计为坏图片。服务重启时遗留活动任务显示为`interrupted`，停止请求会排空已计算队列后安全完成。
