...
- 前端版本标记：`v86`（`app.js` 中的 `APP_VERSION`）。
- 当前稳定发布标签：`v1.3-release`。
- 当前分支：`codex/media-library-cleanup`；已合并最新`origin/main`，尚未合回正式`main`。

## Current implementation state

- 当前业务代码已经完成旧项目功能镜像、工程标准化和轻度死代码清理。
- GitHub 仓库已经发布 `main` 和既有版本标签。
- 正式`main`已发布到`d18a2f2`；当前功能分支包含媒体清理实现和最新main的普通merge，尚未合回正式main。
